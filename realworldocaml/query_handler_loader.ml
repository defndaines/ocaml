...
module Loader = struct
  ...
  let load t handler_name config =
    if Hashtbl.mem t.active handler_name then
      Or_error.error "Cannot re-register an active handler." handler_name String.sexp_of_t
    else
  ...
  type request =
    ...
    | Active_services
        with sexp
  let eval t sexp =
    mathc Or_error.try_with (fun () -> request_of_sexp sexp) with
      | Error _ as err -> err
      | Ok resp ->
          match resp with
...
let () =
  ...
  let loader_instance =
    (module struct
       ...
       in
         Hashtbl.replace loader.Loader.active ~key:Loader.name ~data:loader_instance;
